...
export function Sidebar() {
  ...
  return (
    <aside className="sidebar-shell">
      ...
      <nav className="flex-1 flex flex-col gap-2 w-full items-center lg:items-stretch">
        {navItems.map((item) => {
          const isActive =
            item.href === "/dashboard"
              ? pathname === "/dashboard"
          ...
          return (
            <div key={item.href} className="group relative flex items-center justify-center w-full">
              ...
              {/* Tooltip */}
              <div className="absolute left-[80px] lg:hidden bg-[#121722] border border-white/10 text-gray-200 text-sm font-medium px-4 py-2 rounded-lg opacity-0 translate-x-[-10px] group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 pointer-events-none whitespace-nowrap shadow-xl z-50">
                {item.label}
              </div>
            </div>
          );
      ...
      <div className="p-4 lg:px-3 mt-auto">
        <button
          ...
          aria-label="Sign out"
          className="h-10 w-10 rounded-lg border border-white/10 bg-white/[0.04] flex items-center justify-center text-gray-500 hover:bg-white/[0.08] hover:text-white transition-colors disabled:cursor-not-allowed disabled:opacity-50"
        >
