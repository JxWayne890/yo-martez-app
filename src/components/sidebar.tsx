...
export function Sidebar() {
  ...
  return (
    <aside className="w-[92px] lg:w-[220px] flex flex-col items-center lg:items-stretch py-8 px-3 border-r border-white/5 bg-[#111115]/50 z-20 backdrop-blur-md relative">
      <div className="mb-10 w-full flex justify-center lg:justify-start lg:px-3">
      ...
      <nav className="flex-1 flex flex-col gap-3 w-full items-center lg:items-stretch mt-4">
        {navItems.map((item) => {
          const isActive =
            item.href === "/dashboard"
              ? pathname === "/dashboard"
          ...
          return (
            <div key={item.href} className="group relative flex items-center justify-center w-full">
              <Link
                href={item.href}
                className={`flex items-center justify-center lg:justify-start lg:gap-3 w-[52px] lg:w-full h-[52px] lg:h-12 lg:px-3 rounded-full lg:rounded-xl transition-all duration-300 ease-out ${
                  isActive
              ...
              {/* Tooltip */}
              <div className="absolute left-[80px] lg:hidden bg-[#1a1a24] border border-white/10 text-gray-200 text-sm font-medium px-4 py-2 rounded-xl opacity-0 translate-x-[-10px] group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 pointer-events-none whitespace-nowrap shadow-xl z-50">
                {item.label}
              </div>
            </div>
          );
